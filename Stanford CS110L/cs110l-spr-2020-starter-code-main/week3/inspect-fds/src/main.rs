...
fn main() {
    ...
    let target = &args[1];
	let process = ps_utils::get_target(&target).expect("Error occurs when calling ps or pgrep");
	match process {
		Some(proc) => {
			proc.print();
			let children = ps_utils::get_child_processes(proc.pid).expect("Cannot get list of child processes.");
