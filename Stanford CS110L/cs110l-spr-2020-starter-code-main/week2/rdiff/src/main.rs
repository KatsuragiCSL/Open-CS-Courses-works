	...
	for i in 0..m {
		for j in 0..n {
			if &seq1[i] == &seq2[j] {
				...
				let max_val = cmp::max(grid.get(i + 1, j).unwrap(), grid.get(i, j + 1).unwrap());
				grid.set(i + 1, j + 1, max_val);
			}
		}
	}
...
fn main() {
	...
	let file1 = read_file_lines(filename1).expect("reading file");
	let file2 = read_file_lines(filename2).expect("reading file");

	let lcs_grid = lcs(&file1, &file2);
	print_diff(&lcs_grid, &file1, &file2, file1.len(), file2.len());
}
