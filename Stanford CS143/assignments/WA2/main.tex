...
\begin{tikzpicture}[auto]
    \node[state, initial] (q_0) {};
    \node[state] (q_a1) [above right=3cm and 3cm of q_0] {};
    \node[state] (q_b1) [right=3.2cm of q_0] {};
    \node[state] (q_c1) [below right=3cm and 3cm of q_0] {};
    \node[state] (q_a2) [right of=q_a1] {};
    \node[state] (q_b2) [right of=q_b1] {};
    \node[state] (q_c2) [right of=q_c1] {};
    \node[state, accepting] (q_a3) [right of=q_a2] {};
    \node[state, accepting] (q_b3) [right of=q_b2] {};
    \node[state, accepting] (q_c3) [right of=q_c2] {};
    \path[->]
        (q_0) edge node {$\epsilon$} (q_a1)
        (q_0) edge node {$\epsilon$} (q_b1)
        (q_0) edge node {$\epsilon$} (q_c1)
        (q_a1) edge[loop above] node {$b,c$} (q_a1)
        (q_a1) edge node {$a$} (q_a2)
        (q_a1) edge[bend right=30] node[below] {$a$} (q_a3)
        (q_a2) edge[loop above] node {$b,c$} (q_a2)
        (q_a2) edge node {$a$} (q_a3)
        (q_b1) edge[loop above] node {$a,c$} (q_b1)
        (q_b1) edge node {$b$} (q_b2)
        (q_b1) edge[bend right=30] node[below] {$b$} (q_b3)
        (q_b2) edge[loop above] node {$a,c$} (q_b2)
        (q_b2) edge node {$b$} (q_b3)
        (q_c1) edge[loop above] node {$a,b$} (q_c1)
        (q_c1) edge node {$c$} (q_c2)
        (q_c1) edge[bend right=30] node[below] {$c$} (q_c3)
        (q_c2) edge[loop above] node {$a,b$} (q_c2)
        (q_c2) edge node {$c$} (q_c3);
\end{tikzpicture}
