...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    result = 0
    def helper(big_sum, small_sum, index):
        nonlocal result
        if index == len(s):
            if abs(big_sum - small_sum) <= d:
                result += 1
        else:
            helper(big_sum + s[index], small_sum, index + 1)
            helper(big_sum, small_sum + s[index], index + 1)
    helper(0, 0, 0)
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        tmp = Link(link.first, link.rest) # copy of link
        link.first = value
        link.rest = tmp
