...
filter(x->x*(1/x) != 1, [j for j = 1:1000])
...
ten_twelve = FirstRankOneMatrix([Float64(i) for i in 1:10], [Float64(i) for i in 1:12]) # Your code here
...
RankOneMatrix([i for i in 1:10], [i for i in 1:10])
...
RankTwoMatrix(RankOneMatrix([i for i in 1.0:10.0]), RankOneMatrix([i for i in 0.0:0.1:0.9]))
